...
def animate_traj_qtcoin(traj, robot, pause=True, step_viewer=1, restore=True, callback=None, execute_step_cond=None, frame_time=0.01):
    """make sure to set active DOFs beforehand"""
    if restore: _saver = openravepy.RobotStateSaver(robot)
    # if step_viewer or pause: viewer = trajoptpy.GetViewer(robot.GetEnv())
    for (i,dofs) in enumerate(traj):
        ...
        robot.SetActiveDOFValues(dofs)
        # if pause: viewer.Idle()
        if step_viewer != 0 and (i%step_viewer==0 or i==len(traj)-1): time.sleep(frame_time)
    sys.stdout.write("\n")
